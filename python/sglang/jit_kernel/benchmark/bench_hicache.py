...
def benchmark_one_layer_pf_lf(
    ...
    # page_first host cache: [token, layer, element_size]
    k_cache_src = torch.randn(
    ...
    # layer_first device cache for one layer: [token, element_size]
    k_cache_dst = torch.randn(
        (GPU_CACHE_SIZE, element_size), dtype=DEFAULT_DTYPE, device="cuda"
    )
    v_cache_dst = torch.randn(
    ...
    FN_MAP = {
        "aot": lambda: sglang_aot_transfer_one_pf_lf(
            k_cache_dst, v_cache_dst, indices_dst_gpu,
            k_cache_src, v_cache_src, indices_src_gpu,
            layer_id, element_bytes, src_layout_dim
        ),
        "jit": lambda: sglang_jit_transfer_one_pf_lf(
            k_cache_dst, v_cache_dst, indices_dst_gpu,
            k_cache_src, v_cache_src, indices_src_gpu,
            layer_id, element_size, src_layout_dim
        ),
        "pytorch": lambda: (
            k_cache_dst.__setitem__(indices_dst_gpu, k_cache_src[indices_src_cpu, layer_id].cuda()),
            v_cache_dst.__setitem__(indices_dst_gpu, v_cache_src[indices_src_cpu, layer_id].cuda()),
        ),
    }
...
def benchmark_all_layer_lf_pf(
    ...
    # layer_first device cache: [layer, token, element_size]
    k_caches_src = torch.randn(
        (NUM_LAYERS, GPU_CACHE_SIZE, element_size),
        dtype=DEFAULT_DTYPE, device="cuda"
    )
    ...
    FN_MAP = {
        "aot": lambda: sglang_aot_transfer_all_lf_pf(
            ...
            element_bytes, dst_layout_dim, NUM_LAYERS
        ),
        "jit": lambda: sglang_jit_transfer_all_lf_pf(
            k_cache_dst, v_cache_dst, indices_dst_gpu,
            k_ptrs_src, v_ptrs_src, indices_src_gpu,
            element_bytes, dst_layout_dim, element_bytes
        ),
        "pytorch": lambda: [
            (
                k_cache_dst[indices_dst_cpu, i].__iadd__(
                    k_caches_src[i, indices_src_gpu].cpu() - k_cache_dst[indices_dst_cpu, i]
                ),
                v_cache_dst[indices_dst_cpu, i].__iadd__(
                    v_caches_src[i, indices_src_gpu].cpu() - v_cache_dst[indices_dst_cpu, i]
                ),
            )
            for i in range(NUM_LAYERS)
        ],
