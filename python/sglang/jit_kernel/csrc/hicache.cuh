...
// Source layout: [token, layer, head, dim] - page_first on host
// Dest layout: [token, head, dim] - layer_first single layer on device
template <
    std::integral T,
    std::size_t kElementSize,
    std::size_t kUnroll,
...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void hicache_transfer_per_layer_pf_lf(
  ...
  for (auto i = warp_id; i < length; i += kWorkers) {
    const auto pos_src = static_cast<const T*>(indices_src)[i];
    const auto pos_dst = static_cast<const T*>(indices_dst)[i];
    // page_first: offset = pos * layout_dim + layer_id * element_size
...
// Source layout: [token, head, dim] per layer - layer_first on device (via ptr arrays)
// Dest layout: [token, layer, head, dim] - page_first on host
template <
    std::integral T,
    std::size_t kElementSize,
    std::size_t kUnroll,
...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void hicache_transfer_all_layer_lf_pf(
  ...
  for (auto i = warp_id; i < length; i += kWorkers) {
    ...
    for (std::size_t layer = 0; layer < num_layers; ++layer) {
      ...
      const auto src_k = pointer::offset(k_cache_src, pos_src * kv_cache_src_stride);
      const auto src_v = pointer::offset(v_cache_src, pos_src * kv_cache_src_stride);
      // page_first: offset = pos * layout_dim + layer * element_size
...
struct HiCacheKernel {
  ...
  static void run_one_pf_lf(
    ...
    // Source is page_first buffer, we don't check its strides explicitly
    TensorMatcher({-1, D})  //
        .with_dtype(cache_dtype)
        .with_device<kDLCUDA, kDLCUDAHost, kDLCPU>()
        .verify(k_cache_src)
  ...
  static void run_all_lf_pf(
    ...
    TensorMatcher({N})  //
        ...
        .verify(k_ptr_src)
        .verify(v_ptr_src);
    // Destination is page_first buffer
    TensorMatcher({-1, D})  //
        .with_dtype(cache_dtype)
        .with_device<kDLCUDA, kDLCUDAHost, kDLCPU>()
        .verify(k_cache_dst)
